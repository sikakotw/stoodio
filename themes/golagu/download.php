...
<span class="nxt-author"><?php echo $result['channel']; ?></span> <span class="nxt-date"><?php echo timeline($result['date']); ?></span>
</div>
<div class="nxt-content nxt-wrap-content-single nxt-post">
<p class="nxt-konten media"><img class="media-object pull-left" src="<?php echo $result['image']; ?>?resize=180,180" alt="<?php echo $result['title']; ?>width="120" height="120"> Download <strong><?php echo $result['title']; ?></strong> dengan High Quality Audio MP3 dan HD Video MP4, di-upload oleh <strong></strong>. Dapatkan lagu dan video <strong><?php echo $result['title']; ?></strong> secara gratis, mudah, dan cepat hanya di <strong></strong>!</p>

<audio class="aligncenter" src="https://m.youtube.com/watch?v=<?php echo $result['id']; ?>" type="video/youtube" style="width: 90%; margin-bottom: -5px;">Your browser does not support the audio element</audio>

<div class="download noleft">
<ul class="dl-list">
<li><a class="btn" href="<?php echo convert_permalink($result['id']); ?>" rel="nofollow noopener noreferrer">Download</a></li>
<li><a rel="nofollow" class="btn red" href="https://music.youtube.com/watch?v=<?php echo $result['id']; ?>" target="_blank">Youtube Music</a></li>
...
<blockquote><strong>Peringatan!</strong> Gunakan lagu dari <?php echo site_url(); ?> sebagai preview saja, jika kamu suka dengan lagu <strong><?php echo $result['title']; ?></strong>, lebih baik kamu membeli atau download dan streaming secara legal. Kamu juga bisa mendukung artis dengan mengikuti akun sosial media dan menonton konser mereka.</blockquote>
<div id="info" class="download">
<h4 class="widget-title">Info &amp; Statistik</h4></div>
<div class="info noleft">
<img class="aligncenter" src="<?php echo $result['image']; ?>?resize=320,180" alt="<?php echo $result['title'];?>" width="320" height="180">
<ul>
<li>Judul: <strong><?php echo $result['title']; ?></strong></li>
<li>Video oleh: <strong><?php echo $result['channel']; ?></strong></li>
<li>Waktu putar: <strong><?php echo $result['duration']; ?></strong></li>
<li>Ukuran: <strong><?php echo $result['size']; ?></strong></li>
<li>Audio: <strong>44,1 kHz, Stereo, 128 kb/s, MP3</strong></li>
<li>Dirilis: <strong><?php echo timeline($result['date']); ?></strong></li>
<li>Diperbarui: <strong>20 jam yang lalu</strong></li>
</ul>
</div>
<div id="lirik" class="download">
<h4 class="widget-title">Lirik Lagu</h4></div>
<div class="lirik-full">
Lirik Lagu "<strong><?php echo $result['title']; ?></strong>"<br /><br />
<div id="search-result">
<script> let query = "<?php echo $result['title']; ?>"; </script> <script> let result = document.getElementById("search-result"); function showCard(e) { return `
...
<li>
<a href="<?php echo download_permalink( $item['title'], '--' . $item['id'] ); ?>"><img class="nxt-img" src="<?php echo $item['image'];?>?resize=320,100" alt="<?php echo $item['title'];?>" /> <span class="nxt-title"><?php echo $item['title'];?></span> <span class="duration"><?php echo timeline($item['date']);?></span></a>
</li>
<?php } unset( $item ); ?>
<?php } ?>
</ul>
